...
public class Token extends Model {
    ...
    public enum TypeToken {
        password("reset"), email("email"), admin("admin"), user("user");
    ...
    public void sendNewUserMail(User user, MailerClient mc) throws MalformedURLException {
    	String email = user.getEmail();
        sendMail(user, TypeToken.user, email, mc);
    }
    ...
    private void sendMail(User user, TypeToken type, String email, MailerClient mc) throws MalformedURLException {
        ...
        String urlString = urlString = "https://" + externalServer + "/" + type.urlPath + "/" + token.token;
        ...
        switch (type) {
            ...
            case admin:
                ...
                break;
            case user:
                subject = Messages.get("admin.newuser.subject");
                // Need to update the URL...
                // Replace "admin" with "reset"...
                newURL = url.toString().replace("/user/", "/resetuser/");
                message = Messages.get("admin.newuser.message", newURL);
                toMail = token.email; // == email parameter
                break;
        }
